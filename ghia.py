...
def update_users(action, user_list, dry_run, session, owner, repo, issue_number):
    for username in user_list:
        if not dry_run:
            if update_assignee(action, username, session, owner, repo, issue_number):
...
def ghia(strategy, dry_run, config_auth, config_rules, reposlug):
    ...
    issue_list = get_issue_list(session, owner, repo)
    ...
    for issue in issue_list:
        ...
        click.secho(f"{'/'.join(reposlug)}#{issue['number']} ", nl=False, bold=True)
        ...
        if strategy == "append":
            update_users(ADD, sorted_addable_users, dry_run, session, owner, repo, issue["number"])
            ...
            update_users(REMOVE, sorted_removable_users, dry_run, session, owner, repo, issue["number"])
            update_users(ADD, sorted_addable_users, dry_run, session, owner, repo, issue["number"])
            ...
            if fallback_label:
                if fallback_label in issue_labels:
                    ...
                    if not dry_run:
                        ...
                        r = session.post(f"https://api.github.com/repos/{owner}/{repo}/issues/{issue['number']}/labels",
                                         json={"labels": issue_labels})
                        if r.status_code != 200:
                            write_error(f"Could not update issue {owner}/{repo}#{issue['number']}", 3)
